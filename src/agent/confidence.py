...
def _structural_score(plan: Dict[str, Any]) -> float:
    ...
    branches = sum(1 for _n, c in out_counts.items() if c > 1)
    unresolved = 0
    score = 0.9
    score -= max(0, steps - 4) * 0.05
    score -= branches * 0.10
    score -= min(2, unresolved) * 0.05
...
def _tool_coverage_score(user_text: str, plan: Dict[str, Any]) -> float:
    ...
    implies = {
        ...
        "git": any(k in text for k in ["git", "branch", "commit", "pr", "pull request"]),
    ...
    present = {k: False for k in implies}
...
def _prior_success_score(metrics: Dict[str, Any]) -> float:
    succ = int(metrics.get("prior_successes", 0)); att = int(metrics.get("prior_attempts", 0))
    return (succ + 1) / (att + 2)
...
def compute_plan_confidence(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    w_self = float(getattr(cfg, "conf_w_self", 0.3))
    w_struct = float(getattr(cfg, "conf_w_struct", 0.35))
